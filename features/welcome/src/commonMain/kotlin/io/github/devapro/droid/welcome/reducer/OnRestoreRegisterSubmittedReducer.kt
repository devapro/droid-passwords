...
class OnRestoreRegisterSubmittedReducer(
    private val syncManager: SyncManager
) : Reducer<WelcomeScreenAction.OnRestoreRegisterSubmitted, WelcomeScreenState, WelcomeScreenAction, WelcomeScreenEvent> {

    override val actionClass = WelcomeScreenAction.OnRestoreRegisterSubmitted::class

    override suspend fun reduce(
        action: WelcomeScreenAction.OnRestoreRegisterSubmitted,
        ...
        return when (val result = syncManager.register(action.url.trim(), action.username.trim(), action.password)) {
            ...
            is AppResult.Failure -> Reducer.Result(
                state = loadingState.copy(
                    isRestoreInProgress = false,
                    restoreError = result.error.message ?: "Registration failed"
                )
            )
        }
    }
